...
- **Dungeon identity:** each dungeon spawns only from its own `pool`/`elites` lists and has a signature mechanic — Rust Yards: raider crews (chargers/summons); Labs: energy shields (`shield`/`shmax`, cyan bar absorbed before HP, `hitEnemy` handles it) + hive summons; Spire: snipers `mark:true` set `C.marked` (+40% enemy damage until end of next enemy phase). The victory screen shows each dungeon's `intel` line so the post-boss choice is informed. Keep new enemies inside this structure.
